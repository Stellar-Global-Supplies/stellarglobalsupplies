...
var BATCH_SIZE = Number(process.env.SUPABASE_BATCH_SIZE ?? 500);
...
function materialType(itemName) {
...
function rowKey(parts) {
  return (0, import_crypto.createHash)("sha256").update(parts.map((part) => cleanText(part)).join("|")).digest("hex");
}
function firstDataIndex(record) {
  return record.findIndex((value) => /^\d+$/.test(cleanText(value)));
}
function parseMaster(record, sourceFile, type) {
...
function parseSalesRegister(record, sourceFile) {
  const idx = firstDataIndex(record);
  if (idx < 0) return null;
...
function parsePurchaseRegister(record, sourceFile) {
  const idx = firstDataIndex(record);
  if (idx < 0) return null;
...
function parseSalesItem(record, sourceFile) {
  const idx = firstDataIndex(record);
  if (idx < 0) return null;
  const invoiceNo = cleanText(record[idx + 1]);
  const invoiceDate = parseDate(record[idx + 2]);
...
function parsePurchaseItem(record, sourceFile) {
  const idx = firstDataIndex(record);
  if (idx < 0) return null;
  const invoiceNo = cleanText(record[idx + 1]);
  const invoiceDate = parseDate(record[idx + 2]);
...
function parseRecord(record, fileType, sourceFile) {
  switch (fileType) {
    case "customers":
...
async function upsertRows(table, rows) {
  for (let start = 0; start < rows.length; start += BATCH_SIZE) {
    ...
    const { error } = await supabase.from(table).upsert(batch, { onConflict: conflictColumn(table) });
...
var handler = async (event) => {
  for (const rec of event.Records) {
    ...
    const object = await s3.send(new import_client_s3.GetObjectCommand({ Bucket: bucket, Key: key }));
    if (!object.Body) throw new Error(`S3 object has no body: ${key}`);
    const raw = await readBody(object.Body);
    const records = parseCsv(raw).map((record) => parseRecord(record, fileType, key)).filter((record) => record !== null);
