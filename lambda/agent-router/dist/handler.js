...
var GOOGLE_CLIENT_ID_PARAM = process.env.GOOGLE_CLIENT_ID_PARAM;
var GOOGLE_CLIENT_SECRET_PARAM = process.env.GOOGLE_CLIENT_SECRET_PARAM;
var ALLOWED_ORIGIN = process.env.ALLOWED_ORIGIN ?? "*";
var BEDROCK_MODEL = process.env.BEDROCK_MODEL ?? "amazon.nova-pro-v1:0";
var ANALYTICS_BUCKET = process.env.ANALYTICS_BUCKET ?? "stellar-analytics-reports-471112840461";
var SUPABASE_URL = process.env.SUPABASE_URL;
var SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
var s3Analytics = new import_client_s3.S3Client({ region: REGION });
...
function getBedrockClient() {
  ...
  bedrockClient = new import_client_bedrock_runtime.BedrockRuntimeClient({ region: REGION });
  return bedrockClient;
}
var cachedGoogleClientId = null;
var cachedGoogleClientSecret = null;
async function getGoogleOAuthCredentials() {
  ...
  const [idResult, secretResult] = await Promise.all([
    ssm.send(new import_client_ssm.GetParameterCommand({ Name: GOOGLE_CLIENT_ID_PARAM, WithDecryption: true })),
    ssm.send(new import_client_ssm.GetParameterCommand({ Name: GOOGLE_CLIENT_SECRET_PARAM, WithDecryption: true }))
  ]);
...
async function getGoogleTokenItem(userId) {
...
async function listRecentEmails(accessToken, maxResults = 5) {
...
async function executeGoogleTool(name, args, accessToken) {
  switch (name) {
    case "create_calendar_event": {
      const result = await createCalendarEvent(accessToken, {
        ...
        attendees: Array.isArray(args.attendees) ? args.attendees.map(String) : void 0,
...
async function getSessionMessages(sessionId) {
  const result = await ddb.send(
    new import_lib_dynamodb.QueryCommand({
      ...
      Limit: 40
      // cap history to last 40 messages to stay within context
    })
  );
  return result.Items ?? [];
}
async function persistMessage(msg) {
  const record = {
    ...
    ttl: Math.floor(Date.now() / 1e3) + MESSAGE_TTL_SECONDS
  };
  await ddb.send(
    new import_lib_dynamodb.PutCommand({ TableName: DYNAMODB_TABLE, Item: record })
  );
}
async function fetchBusinessContext() {
  ...
  const topCustomers = (customersResult.data ?? []).map((row) => `${row.customer_name} (\u20B9${Number(row.total_revenue ?? 0).toFixed(0)}, ${Number(row.invoice_count ?? 0)} invoices)`);
  const topSKUs = (skusResult.data ?? []).map((row) => `${row.sku} [${row.material_type ?? "OTHER"}] - \u20B9${Number(row.total_revenue ?? 0).toFixed(0)}, ${Number(row.total_qty ?? 0).toFixed(2)} units`);
  const topSuppliers = (suppliersResult.data ?? []).map((row) => `${row.supplier_name} (\u20B9${Number(row.total_purchase ?? 0).toFixed(0)}, ${Number(row.invoice_count ?? 0)} invoices)`);
  const businessRows = [...businessResult.data ?? []].sort((a, b) => String(a.month).localeCompare(String(b.month)));
  const monthlyLines = businessRows.map(
    (row) => `${row.month}: sales \u20B9${Number(row.sales ?? 0).toFixed(0)}, purchases \u20B9${Number(row.purchases ?? 0).toFixed(0)}, GP \u20B9${Number(row.gross_profit ?? 0).toFixed(0)} (${Number(row.gross_margin_pct ?? 0).toFixed(1)}%)`
  );
  const latestGst = [...gstResult.data ?? []].sort((a, b) => String(a.month).localeCompare(String(b.month))).at(-1);
  const materialLines = (materialResult.data ?? []).map((row) => `${row.material_type}: \u20B9${Number(row.total_revenue ?? 0).toFixed(0)}`);
  const marginLines = (marginResult.data ?? []).map((row) => `${row.item_name}: sales \u20B9${Number(row.sales_amount ?? 0).toFixed(0)}, purchases \u20B9${Number(row.purchase_amount ?? 0).toFixed(0)}, GP \u20B9${Number(row.gross_profit ?? 0).toFixed(0)}`);
  const hasSnap = businessRows.length > 0;
  return {
    ctx: {
      recent_sales_summary: totalInvoices > 0 ? `${totalInvoices} sales invoices analysed from Supabase. Total revenue: \u20B9${totalRevenue.toFixed(2)}. Average invoice value: \u20B9${avgInvoice.toFixed(2)}. Active customers: ${Number(summary?.customer_count ?? 0)}.` : "No sales records have been ingested yet. Ask the user to upload a sales CSV via the Data Ingest section.",
      purchase_summary: totalPurchase > 0 ? `Total purchases: \u20B9${totalPurchase.toFixed(2)} from ${Number(summary?.supplier_count ?? 0)} suppliers.` : "No purchase records have been ingested yet.",
      profit_summary: `Gross profit: \u20B9${grossProfit.toFixed(2)}. Gross margin: ${grossMargin.toFixed(2)}%.`,
      gst_summary: latestGst ? `Latest GST month ${latestGst.month}: output GST \u20B9${Number(latestGst.output_gst ?? 0).toFixed(2)}, input GST \u20B9${Number(latestGst.input_gst ?? 0).toFixed(2)}, net GST \u20B9${Number(latestGst.net_gst ?? 0).toFixed(2)}.` : "No GST line-item data available yet.",
      top_skus: topSKUs.length > 0 ? topSKUs.join("\n") : "No SKU data available yet.",
      top_customers: topCustomers.length > 0 ? topCustomers.join("\n") : "No customer data available yet.",
      top_suppliers: topSuppliers.length > 0 ? topSuppliers.join("\n") : "No supplier data available yet.",
      monthly_revenue: monthlyLines.length > 0 ? monthlyLines.join("\n") : "No monthly Supabase analytics available yet.",
      material_split: materialLines.length > 0 ? materialLines.join(" | ") : "No material split available yet.",
      item_margin: marginLines.length > 0 ? marginLines.join("\n") : "No item margin data available yet.",
...
function emptyBusinessContext(reason) {
  return {
    ctx: {
...
async function handleAnalyticsSummary(event) {
  try {
    ...
    const byMonthAsc = (rows) => [...rows ?? []].sort((a, b) => a.month.localeCompare(b.month));
    const revenueByMonth = byMonthAsc(revenueResult.data).map((row) => ({
      month: row.month,
      revenue: Number(row.revenue ?? 0),
      invoices: Number(row.invoices ?? 0)
    }));
    const businessByMonth = byMonthAsc(businessResult.data).map((row) => ({
      month: row.month,
    ...
    const gstByMonth = byMonthAsc(gstResult.data).map((row) => ({
      month: row.month,
    ...
    const topCustomers = (customersResult.data ?? []).map((row) => ({
      customer_name: row.customer_name,
      total_revenue: Number(row.total_revenue ?? 0),
      invoice_count: Number(row.invoice_count ?? 0)
    }));
    const topSKUs = (skusResult.data ?? []).map((row) => ({
      sku: row.sku,
      total_revenue: Number(row.total_revenue ?? 0),
      total_qty: Number(row.total_qty ?? 0),
      material_type: row.material_type ?? "OTHER"
    }));
    const topSuppliers = (suppliersResult.data ?? []).map((row) => ({
      supplier_name: row.supplier_name,
      total_purchase: Number(row.total_purchase ?? 0),
      invoice_count: Number(row.invoice_count ?? 0)
    }));
    const itemMargin = (marginResult.data ?? []).map((row) => ({
    ...
    const materialSplit = (materialResult.data ?? []).reduce(
      (acc, row) => {
        const key = String(row.material_type ?? "OTHER");
    ...
    const growthRate = firstMonth && lastMonth && firstMonth.sales > 0 ? (lastMonth.sales - firstMonth.sales) / firstMonth.sales * 100 : 0;
    const period = revenueByMonth.length > 0 ? `${revenueByMonth[0].month} to ${revenueByMonth[revenueByMonth.length - 1].month}` : `Last ${months} months`;
    const summary = {
      period,
...
async function handleGoogleDisconnect(event) {
...
async function handleAgentChat(agentId, event) {
  const body = parseBody(event);
  if (!body) {
  ...
  const sessionId = session_id ?? `${agentId}_${(0, import_crypto.randomUUID)()}`;
  ...
  try {
    ...
    businessContext = emptyBusinessContext(message2);
  }
  const { ctx, meta } = businessContext;
  const historyRecords = await getSessionMessages(sessionId);
  const userMsgId = (0, import_crypto.randomUUID)();
  ...
  const bedrockMessages = historyRecords.map((m) => ({
  ...
  try {
    ...
    let bedrockResponse = await bedrock.send(new import_client_bedrock_runtime.ConverseCommand(converseRequest));
    let rounds = 0;
    while (bedrockResponse.output?.message?.stopReason === "tool_use" && rounds < 5) {
      rounds++;
      const toolResults = [];
      ...
      if (assistantMessage.content) {
        for (const block of assistantMessage.content) {
          if (block.toolUse) {
            ...
            const fnArgs = block.toolUse.input ?? {};
            ...
            try {
              ...
              console.error("[agent-router] Tool execution failed", { fnName, error: toolErr.message });
      ...
      bedrockResponse = await bedrock.send(new import_client_bedrock_runtime.ConverseCommand(continueRequest));
    }
    const finalMessage = bedrockResponse.output?.message;
    if (finalMessage?.content) {
      const textParts = finalMessage.content.filter((block) => "text" in block).map((block) => block.text);
  ...
  const assistantMsgId = (0, import_crypto.randomUUID)();
  const assistantTs = (/* @__PURE__ */ new Date()).toISOString();
  await persistMessage({
    entityType: "MESSAGE",
    session_id: sessionId,
...
async function readAnalyticsS3(s3Key) {
  const result = await s3Analytics.send(
    new import_client_s3.GetObjectCommand({ Bucket: ANALYTICS_BUCKET, Key: s3Key })
  );
...
async function readFirstAnalyticsS3(keys) {
  let lastError;
...
async function handleWebAnalytics(event) {
...
async function handleMetaAnalytics(event) {
  const period = event.queryStringParameters?.period ?? "weekly";
  const keys = period === "monthly" ? ["meta/monthly.json", "meta_monthly.json", "monthly.json"] : ["meta/weekly.json", "meta_weekly.json", "weekly.json"];
  try {
